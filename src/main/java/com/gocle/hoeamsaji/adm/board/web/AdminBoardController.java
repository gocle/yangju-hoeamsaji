package com.gocle.hoeamsaji.adm.board.web;
...
import com.gocle.hoeamsaji.adm.board.service.AdminBoardService;
import com.gocle.hoeamsaji.adm.board.vo.BoardArticleVO;
import com.gocle.hoeamsaji.adm.board.vo.BoardConfigVO;
import com.gocle.hoeamsaji.adm.board.vo.BoardReplyVO;
import com.gocle.hoeamsaji.adm.code.service.AdminCodeService;
import com.gocle.hoeamsaji.adm.code.vo.CodeVO;
...
public class AdminBoardController {
	...
	public String configList(@ModelAttribute("form") BoardConfigVO boardConfigVO, ModelMap model) throws Exception {
		
		String menuId = boardConfigVO.getMenuId();
		List<BoardConfigVO> resultList = adminBoardService.listBoardConfig(boardConfigVO);
		
		int totalCnt = 0;
		Integer pageSize = boardConfigVO.getPageSize();
		Integer pageIndex = boardConfigVO.getPageIndex();
        ...
        paginationInfo.setPageSize(boardConfigVO.getPageUnit());
        paginationInfo.setTotalRecordCount(totalCnt);
        
        model.addAttribute("paginationInfo", paginationInfo);
        model.addAttribute("resultCnt", totalCnt);
        model.addAttribute("resultList" , resultList);
        model.addAttribute("topresult", resultList);
        model.addAttribute("menuId", menuId);
        
        return "/adm/bbs/config/configList";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("config/form.do")
	public String insertForm(@ModelAttribute("form")BoardConfigVO boardConfigVO, ModelMap model) throws Exception {
		
		String menuId = boardConfigVO.getMenuId();
		List<CodeVO> codeList = adminCodeService.codeUpperList();
		...
		return "adm/bbs/config/configForm";
	}
	...
	public String insertBbsConfig(@ModelAttribute("form")BoardConfigVO boardConfigVO, HttpSession session, RedirectAttributes redirectAttributes) throws Exception {
		
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.putSessionToVo(boardConfigVO);
		
		String menuId = boardConfigVO.getMenuId();
		String retMsg = "";
		
		//thumbnail = 1
		if(boardConfigVO.getBcId().equals("gallery")) {
			boardConfigVO.setBcSupportThumbnail("1");
		}
		
		int result = adminBoardService.insertBoardConfig(boardConfigVO);
		if(result > 0) {
			retMsg = "등록되었습니다.";
		} else {
			retMsg = "등록할 수 없습니다.";
		...
		return "redirect:/adm/bbs/config/list.do?menuId="+menuId;
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("config/detail.do")
	public String detailBbsConfig(@ModelAttribute("form")BoardConfigVO boardConfigVO, RedirectAttributes redirectAttributes, ModelMap model) throws Exception {
		
		String menuId = boardConfigVO.getMenuId();
		
		List<CodeVO> codeList = adminCodeService.codeUpperList();
		
		BoardConfigVO result = adminBoardService.selectBoardConfig(boardConfigVO);
	...
	public String updateBbsConfig(@ModelAttribute("form")BoardConfigVO boardConfigVO, RedirectAttributes redirectAttributes, ModelMap model) throws Exception {
		...
		int result = adminBoardService.updateBoardConfig(boardConfigVO);
		if(result > 0) {
			retMsg = "수정되었습니다.";
		} else {
			retMsg = "수정할 수 없습니다.";
	...
	public String deleteBbsConfig(@ModelAttribute("form") BoardConfigVO boardConfigVO, RedirectAttributes redirectAttributes, ModelMap model) throws Exception {
		...
		int result = adminBoardService.deleteBoardConfig(boardConfigVO);
		if(result > 0) {
			retMsg = "삭제되었습니다.";
		} else {
			retMsg = "삭제할 수 없습니다.";
	...
	public String boardInsert(@PathVariable("bcId") String bcId, @ModelAttribute("form") BoardConfigVO boardConfigVO, BoardArticleVO boardArticleVO
		...
		if("1".equals(boardConfigVO.getBcSupportThumbnail())) {
			...
			if(!Arrays.asList("jpg", "jpeg", "png").contains(ext)) {
				...
				redirectAttributes.addFlashAttribute("retMsg", retMsg);
				return "redirect:/adm/bbs/" + bcId + "/form.do?menuId="+menuId;
			}
		}
		...
		return "redirect:/adm/bbs/" + bcId + "/list.do?menuId="+menuId;
	}
	...
	public String boardDetail(@PathVariable("bcId")String bcId, @RequestParam(value="brIdResult", required=false)String brId, BoardConfigVO boardConfigVO, @ModelAttribute("form")BoardArticleVO boardArticleVO,
		...
		return "/adm/bbs/" + boardConfigVO.getBcType() + "/view";
	}
...
/*		
		...
		System.out.println("multirequest" +file.getOriginalFilename());
			*/
			...
			if( boardReplyVO == null ){
				...
				return "redirect:/adm/bbs/"+boardConfigVO.getBcId()+"/detail.do";
		...
		returnPage = "/adm/bbs/"+boardArticleVO.getBcId()+"/detail.do?menuId="+menuId+"&baId="+boardReplyVO.getBaId();
	...
	public String replyDeleteGet(
		...
		returnPage = "/adm/bbs/"+boardArticleVO.getBcId()+"/detail.do?baId="+boardArticleVO.getBaId()+"&menuId="+menuId;
	...
	public String updateReplyPost(
		...
		returnPage = "/adm/bbs/"+bcId+"/detail.do?baId="+result.getBaId()+"&menuId="+menuId;
