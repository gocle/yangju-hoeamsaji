...
<div class="wrapper nav-default" >
	<header>
		<a href="#" class="logo"><img src="/hoeamsaji/assets/site/hoeamsaji/img/common/ico_logo.png" alt="Yangju 양주시"></a>
		<div>
								...
								<c:choose>
									...
									<div id="div_${menu1.menuId }">
										<c:forEach var="menu2" items="${menuList[key2]}" varStatus="status2">
											<!-- S : depth-2 -->
													<c:choose>
														<c:when test="${0 eq menu2.isLeafMenu}"><a id="a_${menu2.menuId }" href="#" style="display: table;">${menu2.menuTitle }</a></c:when>
														<c:otherwise><a id="a_${menu2.menuId }" style="display: table;" <c:if test="${menu2.isLeaf eq 'true' }" >href="#"</c:if><c:if test="${menu2.isLeaf eq 'false' }" >href="${menu2.menuUrl}?menuId=${menu2.menuId}"</c:if> >${menu2.menuTitle }</a></c:otherwise>
													</c:choose>
										</c:forEach>
									</div>
									</c:when>
