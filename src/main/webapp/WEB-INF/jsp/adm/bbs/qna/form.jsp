...
function fn_list() {
		...
		$("#searchform").attr("action", "${contextRoot}/adm/bbs/"+bcId+"/list.do");
								...
								<c:forEach var="fileVO" items="${fileList}" varStatus="status">
									<c:if test="${fileVO.thumbnailCrop eq 'N'}"> <!-- 썸네일 여부가 0 일때..  -->
										<span>
											...
											<button type="button" onclick="fn_egov_deleteFile('${fileVO.atchFileIdx}','${contextRoot}/adm/bbs/${boardConfigVO.bcId}/insert.do?baId=${boardArticleVO.baId}');">삭제</button>
										</span>
									</c:if>
								</c:forEach>
