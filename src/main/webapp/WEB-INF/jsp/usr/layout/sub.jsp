...
<div class="breadcrumbs">
  <a href="/hoeamsaji/usr/main.do" class="home">홈</a>
        ...
        <c:forEach var="menu" items="${listTopMenu}">
          ...
          <c:set var="separator1" value="${fn:endsWith(menuPathList[0], '.do') ? '?' : '&'}" />
           <c:choose>
			  ...
			  <c:otherwise>
			    <a href="/hoeamsaji/${menuPathList[0]}${separator1}menuId=${menuPathList[1]}"
			       target="_self">
			      ${menu.menuTitle}
			    </a>
			  </c:otherwise>
          ...
          <c:forEach var="menu" items="${listSubMenu}">
            ...
            <c:set var="separator2" value="${fn:endsWith(menu.menuUrl, '.do') ? '?' : '&'}" />
              <c:choose>
				  ...
				  <c:otherwise>
				    <a href="/hoeamsaji/${menu.menuUrl}${separator2}menuId=${menu.menuId}"
				       target="_self"
				       class="${menu.menuId eq menuVO.menuId ? 'active' : ''}">
				      ${menu.menuTitle}
				    </a>
          ...
          <c:forEach var="menu" items="${listSubMenu}">
            ...
            <c:set var="separator3" value="${fn:endsWith(menu.menuUrl, '.do') ? '?' : '&'}" />
              <c:choose>
				  ...
				  <c:otherwise>
				    <a href="/hoeamsaji/${menu.menuUrl}${separator3}menuId=${menu.menuId}"
				       target="_self"
				       class="${menu.menuId eq menuVO.menuId ? 'active' : ''}">
				      ${menu.menuTitle}
				    </a>
