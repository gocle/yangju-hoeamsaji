...
(function(factory) {
            ...
            _.initials = {
                ...
                dates: {
					...
					ko: {
                        ...
                        noEventForToday: "선택한 날짜에 일정이 없습니다.",
                        noEventForThisDay: "선택한 날짜에 일정이 없습니다.",
                        previousYearText: "이전 연도",
                        nextYearText: "다음 연도",
                        closeSidebarText: "Close sidebar",
                        closeEventListText: "Close event list"
                    }
    ...
    EvoCalendar.prototype.formatDate = function(date, format, language) {
        ...
        var ndate = (date instanceof Date) ? date : new Date(date);
		...
		if (typeof date === "string") {
		    ...
		    if (parts.length === 3) {
		        ndate = new Date(parts[2], parts[0]-1, parts[1]);
		    } else {
		        ndate = new Date(date);
		    }
		} else {
...
/*        markup += '<div class="event-icon"><div class="event-bullet-'+event_data.type+'"';
        ...
        markup += '</p>'
        if (event_data.description) markup += '<div class="event-desc">'+event_data.description+'</div>';
        markup += '</div>';
        markup += '</div>';
        eventListEl.append(markup);
